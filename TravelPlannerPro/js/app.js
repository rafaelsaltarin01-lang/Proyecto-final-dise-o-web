...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // Carga el historial guardado en la interfaz apenas abre la página
    if (typeof renderHistoryModule === 'function') {
        renderHistoryModule();
    }
});
...
function setupEventListeners() {
    ...
    //  Asegurar captura correcta de los inputs
    if (registerForm) {
...
async function handleSearch() {
    ...
    try {
        ...
        // Se ejecuta el guardado automático con la fecha y hora estructuradas
...
function saveToHistory(name) {
    ...
    const now = new Date();
    const dateStr = now.toLocaleDateString('es-CO'); // Formato DD/MM/AAAA
    const timeStr = now.toLocaleTimeString('es-CO', { hour: '2-digit', minute: '2-digit', second: '2-digit' }); // Formato HH:MM:SS

    // Inserta la consulta al inicio para mostrar primero lo más reciente
    history.unshift({ 
        country: name, 
        date: dateStr, 
        time: timeStr 
...
// Controlamos que el historial guarde un máximo de 10 registros
    if (history.length > 10) history.pop();

    localStorage.setItem('query_history', JSON.stringify(history));
    updateDashboardCounters();

    // Actualiza la visualización de la tabla del historial inmediatamente si la función existe
    if (typeof renderHistoryModule === 'function') {
        renderHistoryModule();
    }
}
...
function renderHistoryModule() {
    ...
    // Leemos el historial desde el LocalStorage
    const history = JSON.parse(localStorage.getItem('query_history')) || [];

    // Si el historial está vacío
    if (history.length === 0) {
        container.innerHTML = `<p class="no-history-text">No has realizado ninguna búsqueda todavía.</p>`;
        return;
    }

    // Creamos la estructura usando clases CSS limpias
    let html = `
        <table class="history-table">
            <thead>
                <tr>
                    <th>📅 Fecha</th>
                    <th>⏰ Hora</th>
                    <th>🌐 País Consultado</th>
                </tr>
            </thead>
            <tbody>
    `;

    // Recorremos cada consulta guardada automáticamente
    ...
    html += `
        ...
        <button onclick="clearSearchHistory()" class="btn-clear-history">🗑️ Limpiar Historial</button>
    `;

    container.innerHTML = html;
}

// Función global para vaciar por completo el historial
